...
(deftest initialize-db-sets-defaults
  (testing ":initialize-db populates every fact the Svelte scaffold held"
    ...
    (is (= "Ai etzhayyim Project Saikin" @(rf/subscribe [:app/title])))
    (is (= "etzhayyim-project-saikin" @(rf/subscribe [:app/name])))
    (is (= "etzhayyim-project-saikin" @(rf/subscribe [:app/project])))
    (is (= "cloudflare surface" @(rf/subscribe [:app/kind])))
    (is (= 2 @(rf/subscribe [:app/route-count])))
    (is (= ["s41k1n01.etzhayyim.com/*" "saikin.etzhayyim.com/*"]
           @(rf/subscribe [:app/routes])))
    (is (= 8 (count @(rf/subscribe [:app/vars]))))
    (is (false? @(rf/subscribe [:app/xrpc?])))
    (is (= "cljs/src/saikin/app.cljs" @(rf/subscribe [:app/relative-path])))))
