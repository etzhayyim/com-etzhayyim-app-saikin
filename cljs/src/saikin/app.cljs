(ns saikin.app
  "app-saikin appview — reagent + re-frame, view built from jp-go-dds
  (デジタル庁デザインシステム) hiccup.

  Faithful port of the previous SvelteKit scaffold's status page
  (`svelte/src/routes/+page.svelte`, 68 lines): a static display of this
  ...
  in its <script> block, with three exceptions documented below.
  ...
  - `:app/route-count` and `:app/routes` now report what `wrangler.jsonc`
    actually declares (2 route patterns: `s41k1n01.etzhayyim.com/*` and
    `saikin.etzhayyim.com/*`), not the stale `0` / `[]` the Svelte
    constant carried.
  - `:app/xrpc?` is now false, even though the Svelte constant carried
    `true`. This migration's wrangler.jsonc drops `main` (see that
    file's header comment and the repo README): neither Worker source in
    ...
    preserved byte-for-byte at `src/xrpc-dispatcher.ts` (moved, not
    deleted, from `svelte/src/routes/xrpc/[...path]/+server.ts`) —
    backend Worker/XRPC code is out of scope for a frontend migration.
  - `:app/relative-path` now names this file
    (`cljs/src/saikin/app.cljs`), not the deleted Svelte one
    (`60-apps/etzhayyim-project-saikin/svelte/src/routes/+page.svelte`).
  ...
  components or ext-rules change) with:
    ...
    (spit \"public/index.html\"
          (page/->page {:title \"etzhayyim-project-saikin\"
                         :lang \"ja\"
                         :description \"saikin (細菌) — bacteria horizontal transfer layer appview (reagent + re-frame + jp-go-dds).\"
                         :css (slurp (io/resource \"jp_go_dds/dds.css\"))}
                        [:div {:id \"app\"} \"etzhayyim-project-saikin loading…\"]
...
(def default-db
  {:app/title "Ai etzhayyim Project Saikin"
   :app/project "etzhayyim-project-saikin"
   :app/name "etzhayyim-project-saikin"
   :app/kind "cloudflare surface"
   :app/route-count 2
   :app/routes ["s41k1n01.etzhayyim.com/*" "saikin.etzhayyim.com/*"]
   :app/vars ["AGENTGATEWAY_MCP_ROUTER_URL" "APP_CAPABILITIES" "APP_DESCRIPTION"
              ...
              "APP_UI_TYPE"]
   :app/xrpc? false
   :app/relative-path "cljs/src/saikin/app.cljs"})
